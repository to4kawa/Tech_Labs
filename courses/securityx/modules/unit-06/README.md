...
## 1. 目的（5分）
- SLO・冗長化・バックアップ・復旧手順を無料範囲で設計し、チェックリスト化する。
...
### 4.4 検証ログ確認
```bash
gcloud logging read 'protoPayload.methodName:"storage.objects" OR protoPayload.methodName:"backup"' --limit=5 --format='table(timestamp,protoPayload.methodName,protoPayload.resourceName)'
```
期待結果例: コマンドが正常終了し、指定ファイルまたはログが確認できる。

### 4.5 チェックリスト化
```bash
cat > u06_checklist.md <<'EOF'
- [ ] SLO合意済み
- [ ] バックアップ頻度定義済み
- [ ] 復旧訓練記録あり
- [ ] 障害連絡手順あり
